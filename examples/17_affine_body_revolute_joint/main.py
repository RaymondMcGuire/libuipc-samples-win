...
from uipc.geometry import SimplicialComplexIO, SimplicialComplex, label_surface, ground
...
pos0s = np.array([[-0.6, 0, -1],
                  [0.6, 0, -1]], dtype=np.float32)
pos1s = np.array([[-0.6, 0, 1],
                  [0.6, 0, 1]], dtype=np.float32)
joint_mesh = abrj.create_geometry(
    pos0s, pos1s,
    [left_geo_slot, mid_geo_slot], [0, 0],
    [mid_geo_slot, right_geo_slot], [0, 0],
    [100.0, 100.0],
)
